...
Integrates AI-powered answer relevance with emotion, fluency, and sentiment scoring
"""
...
class ScoringEngine:
    ...
    1. AI Relevance Score (40%) - from Claude API
    ...
    def __init__(self):
        ...
        # Updated weights to prioritize AI relevance
        self.weights = {
    ...
    def calculate_final_score(
        ...
        Args:
            ai_relevance_score: 0-100 score from AI evaluator
...
if __name__ == "__main__":
    print("📊 Enhanced Scoring Engine - Testing with AI Integration")
    print("=" * 70)
...
# Called directly by interview_app.py and evaluation_pipeline.py.
# The ScoringEngine class above is kept intact for backward compatibility.
# ══════════════════════════════════════════════════════════════════════════════

# Emotion → confidence numeric score (Task 4)
CONFIDENCE_MAP = {
    "confident": 90,
    "neutral":   70,
    "nervous":   50,
...
# Grade thresholds: (min_score, grade, label)
_GRADE_THRESHOLDS = [
    (90, "A+", "Outstanding"),
    (80, "A",  "Excellent"),
    (70, "B",  "Good"),
...
def calculate_esi(emotions: list) -> float:
    """
    Emotion Stability Index: measures consistency of emotion across answers.
    ESI = 1 − (std_dev_of_emotion_scores / 100)
    Higher is better (more emotionally stable delivery).
    """
    if not emotions:
        return 0.0
...
def generate_report(
    ...
    """
    Build a standardised report dict for one evaluated answer.

    Final score formula (Task 4):
        final = relevance * 0.40 + fluency * 0.30 + confidence * 0.30

    Args:
        ai_scores : optional dict with keys clarity, structure, depth (Task 3)

    Returns:
        dict with keys: scores, emotion, grade, grade_label, feedback,
                        and optionally ai_clarity, ai_structure, ai_depth
    """
    # Confidence from emotion map
    confidence = CONFIDENCE_MAP.get(emotion, 70)

    # Weighted final score
    final = round(relevance * 0.40 + fluency * 0.30 + confidence * 0.30, 1)
    ...
    # Attach AI sub-scores when available (Task 3)
    if ai_scores:
